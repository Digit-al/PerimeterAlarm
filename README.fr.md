...
4. **Sonnerie & vibration** (spécifique à l'alarme) :
   ...
   - Le son de l'alarme est routé vers la **sortie connectée** : quand des écouteurs filaires ou Bluetooth sont branchés, il joue sur le **flux média** (`USAGE_MEDIA`), qui est garanti de les atteindre ; sinon il joue sur le flux **volume alarme** système (`USAGE_ALARM`) — indépendant du volume média, et audible même en mode silencieux.
   - Pendant que l'alarme sonne dans les écouteurs, le **volume média système** est temporairement mis au maximum — seulement après que la musique est effectivement arrêtée (focus audio accordé) — pour que le slider corresponde exactement au niveau perçu. Le volume d'origine est restauré avant de libérer le focus, pour que la musique reprenne à son volume d'origine.
   - Tant que l'alarme sonne, l'application détient un **focus audio transitoire** sur le flux actif : la musique en cours se met en pause et **reprend automatiquement une fois l'alarme arrêtée**. Si la sortie change pendant que l'alarme sonne (ex. branchement des écouteurs), le lecteur bascule automatiquement sur le bon flux (vérifié à chaque cycle de surveillance).
...
Quand aucune alarme n'est active et dans sa période, le service **dort jusqu'au prochain début de période** (calculé à partir des jours + heures, jusqu'à 8 jours devant) au lieu de poller toutes les 30 s. Sauvegarder une alarme réveille le service immédiatement. Pour rendre ce sommeil fiable, le service (re)planifie un **réveil résistant au Doze** (`AlarmManager.setAlarmClock`) sur le *début exact* de la prochaine période — **pour n'importe quel sommeil, pas seulement les longs** — et **plafonne chaque sommeil à 2 h** : au-delà, il se réveille par paliers, re-évalue l'état et re-planifie l'alarme — un réveil Doze décalé (ou une alarme perdue) est donc rattrapé à l'étape suivante et l'alarme exacte est rafraîchie. Le réveil est aussi re-planifié quand le service est détruit (un service tué dont le processus survit orphelinerait sinon le sommeil). L'alarme se déclenche à l'heure même si l'appareil est en mode Doze, et une icône horloge est affichée dans la barre d'état tant que le réveil est en attente. Sur Android 12+ (ciblant le SDK 31+), cela nécessite la permission `SCHEDULE_EXACT_ALARM` : elle est **refusée par défaut pour les apps ciblant le SDK 33+** (on cible le 34), l'utilisateur l'accorde donc en un geste : l'app affiche **automatiquement une carte d'avertissement sur l'écran d'accueil** (et une carte d'état dans Réglages) dont le bouton ouvre l'écran système dédié à l'app. Si elle manque, l'app vérifie `canScheduleExactAlarms()` et dégrade gracieusement en sommeil inexact simple plafonné.
...
5. pendant les sommeils entre périodes, plafonne chaque sommeil à **2 h** et (re)planifie un réveil résistant au Doze sur le début exact de la prochaine période via `AlarmManager.setAlarmClock` (géré par `WakeReceiver`) — auto-corrigé : un réveil décalé/perdu est rattrapé à l'étape 2 h suivante et re-planifié.
